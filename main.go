...
func main() {
	...
	host := flag.String("host", "127.0.0.1", "the database host")
	port := flag.Int("port", 3306, "the database port")
	user := flag.String("user", "root", "the database user")
	password := flag.String("password", "Shraddha99", "the database password")
	schema := flag.String("schema", "testdb", "the database schema")
